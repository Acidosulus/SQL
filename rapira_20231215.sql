...
--DECLARE @Date1 datetime = '2023-12-01 00:00:00';
--DECLARE @Date2 datetime = '2023-12-31 23:59:59';
--PreviousDay int
...
WHERE 1=1
	and b.billing_month between '2023-11-01' and '2023-11-30'
	and b.Connection_id = 1
	and b.is_deleted = false
	and dbt.code_nsi in (-1, 11)
	and COALESCE(d.analytics, dp.analytics) IN ('') 
	and CASE
		WHEN d.pay_deadline is null
		THEN true
		ELSE d.pay_deadline <= ('2023-11-30'::timestamptz - interval '1 day') END
		= true
...
ON
 ...
 WHERE 1=1
 	and d.document_at between '2023-11-01' and '2023-11-30'
	and d.is_deleted = false
...
JOIN
 ...
 WHERE 1=1
	...
	and CASE
		...
		THEN true
		ELSE doc1.pay_deadline <= ('2023-11-30'::timestamptz - interval '1 day') END
		= true
;
...
SELECT 
	...
	LEAST(max(month_pay)
		, '2023-11-01'::date - interval '1 month') 
		as month_pay_to,
...
WHERE 1=1
	and (d.terminate_at < '2023-11-30'
		or COALESCE(d.end_at, '2023-11-01') < '2023-11-30'
		)
...
WHERE 1=1
	and (date_part('year', '2023-11-01'::date) - date_part('year', a.billing_month::date)) * 12
		+ (date_part('month', '2023-11-01'::date) - date_part('month', a.billing_month::date)) 
		>=33 -- 33 месяца
...
ON
	...
	and (date_part('year', l.created_at::date) - date_part('year', '2023-11-01'::date)) * 12
		+ (date_part('month', l.created_at::date) - date_part('month', '2023-11-01'::date)) <= 12
...
ON
	...
	and (COALESCE(cwl.date_from, '00010101') <= '2023-11-01')
	and (COALESCE(cwl.date_to, '99990101') >= '2023-11-01')
...
ON
	...
	and (COALESCE(cbi.date_from, '00010101') <= '2023-11-01')
	and (COALESCE(cbi.date_to, '99990101') >= '2023-11-01')
